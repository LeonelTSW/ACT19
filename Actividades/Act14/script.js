...
function resolverEcuacionParseada() {
    ...
    try {
        ...
        // NOTA: 'math.parse' y 'math.simplify' provienen de la librería math.js
        ...
        // Verifica que la ecuación no sea trivial (ej: 0=0 o 5=0)
        if (coeficientes.a === 0) {
            if (coeficientes.b === 0) {
                 throw new Error("Ecuación trivial (ej: 0=0). Infinidad de soluciones.");
            } else {
                 throw new Error(`Ecuación inconsistente (ej: ${coeficientes.b}=0). No tiene solución.`);
            }
        }
        ...
        resBox.innerHTML = `
            ...
            <div class="nota-didactica">
                El método mueve todos los términos a un lado para simplificar. La solución de $ax + b = 0$ es $x = -b/a$.
            </div>
        `;
    } catch (error) {
...
function extraerCoeficientes(ecuacion) {
    ...
    if (sinX) {
        try {
            // Utilizamos math.evaluate para calcular la constante si es una expresión (ej: 5+3)
...
function extractVariables(formulaStr) {
    // Busca secuencias de letras y números (ej. 'F', 'pOH', 'r1')
    const matches = formulaStr.match(/[a-zA-Z][a-zA-Z0-9]*/g);
    // Excluir números y operadores conocidos si la fórmula es compleja
    if (!matches) return [];
...
function despejarVariable(expresionOriginal, variableObjetivo, variableDespejada) {
    try {
        ...
        if (matchComplejo && matchComplejo[2] === variableObjetivo) {
            ...
            // 1. Multiplicar por el divisor: varDespejada * divisor
            // 2. Dividir por el coeficiente: (varDespejada * divisor) / coef1
            // 3. Sumar/restar la constante: (resultado_anterior) + constante
        ...
        // CASO: Suma o resta simple (A = B + C)
        ...
        if (expresion.includes('/')) {
            ...
            if (numerador.includes(variableObjetivo)) {
                ...
                if (numerador === variableObjetivo) {
                    // Caso: varObjetivo / denominador
                    return `(${variableDespejada}) * (${denominador})`;
                } else {
                    // Caso: Multiplicación en numerador: a*b*varObjetivo / denominador
                    ...
                    if (otrosFactores.length === 0) {
                        ...
                        // Multiplicar por el denominador, dividir por los otros factores
                ...
                if (denominador === variableObjetivo) {
                    // Caso: numerador / varObjetivo
                    return `(${numerador}) / (${variableDespejada})`;
                } else {
                    // Caso: Multiplicación en denominador
                    ...
                    if (otrosFactores.length === 0) {
                        ...
                        // Intercambio: variableObjetivo = numerador / (variableDespejada * otrosFactores)
        ...
        // CASO: Multiplicación simple (A = B * C * D)
        if (expresion.includes('*')) {
        ...
        // CASO: Variable sola (A = B)
        if (expresion === variableObjetivo) {
            return variableDespejada;
        }
        
        // Si no se encuentra un patrón de despeje simple, devolver la expresión original
        return expresion; 
...
function despejarSumaResta(expresion, variableObjetivo, variableDespejada) {
    ...
    let factor = 1;
    let tieneCoeficiente = false;
    
    // Caso: tiene coeficiente (ej: 2*var)
    if (terminoConVariable.texto.includes('*')) {
        const partes = terminoConVariable.texto.split('*');
        const coefStr = partes.find(p => p !== variableObjetivo);
        try {
            factor = math.evaluate(coefStr);
            tieneCoeficiente = true;
        } catch(e) { /* Ignorar, no es un coeficiente numérico simple */ }
    }

    // Aplicar el factor y el signo del término
    ...
    } else if (terminoConVariable.signo === '-') {
        // Si el término era -Var, entonces Var = -(Resultado)
        resultado = `-(${resultado})`;
    }
...
/**
 * Parsea una expresión en términos con sus signos (respetando paréntesis)
 */
function parsearTerminos(expresion) {
    ...
    for (let i = 0; i < expresion.length; i++) {
        ...
        if (char === '(') {
            ...
            // Es un operador de suma/resta principal
            terminos.push({ signo: signo, texto: cleanString(actual) });
            signo = char;
            actual = '';
        } else if ((char === '+' || char === '-') && nivel === 0 && actual === '') {
            // Es el signo inicial de la expresión (+ o -)
            signo = char;
        } else {
            actual += char;
        }
    ...
    // El primer término si es positivo no tiene el signo inicial. Ajustar
    if (terminos.length > 0 && expresion.trim()[0] !== '-' && expresion.trim()[0] !== '+') {
         terminos[0].signo = '+';
    }
...
function dividirPorOperadorPrincipal(expresion, operador) {
    ...
    return [cleanString(parte1), cleanString(parte2)];
}
...
function extraerFactoresMultiplicacion(expresion) {
    ...
    for (let i = 0; i < expresion.length; i++) {
        ...
        if (char === '(') {
            ...
            if (actual) {
                factores.push(cleanString(actual));
                actual = '';
            }
        } else {
            actual += char;
...
function procesarFormulaLiteral() {
    ...
    try {
        ...
        for (const variable of variables) {
            ...
            // Si la variable a despejar ya está sola en el lado izquierdo
            if (ladoIzq === variable) {
                formulaDespejada = `${variable} = ${ladoDer}`;
            } else {
                // Despejar la variable del lado derecho, usando el lado izquierdo como resultado
        ...
        htmlTable += `
            ...
            <div class="nota-didactica">
                ...
                Soporta suma, resta, multiplicación, división y expresiones con paréntesis para casos simples.
            </div>
        `;
